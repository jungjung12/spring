...
<body>
			...
			*/
			 ...
			 */
			 ...
			 function detail(e) {
				 ...
				 console.log($(e).find('.thumbnail-img').arrt('src'));
				 console.log($(e).find('.thumnail-title').text());
				 console.log($(e).find('.text-muted').text());
				 */
	...
	<div class="container">
		...
		<section class="bg-light" id="portfolio" style="padding-top: 90px; padding-bottom: 0;">
			<div class="container">
				<div class="row">
				<!-- 모달에 뿌릴 정보까지 한번에 조회 후 hidden 처리를 한 뒤에 모달의 요소에 해당하는 값을 뿌려줌 -->
			<c:forEach items="${board }" var="img">
					...
					<input type="hidden" class="thumbnail-content" value="${img.boardContent }" />
						<a class="portfolio-link" data-toggle="modal" href="#MyModal">
							...
							<img style="height:70%;" class="img-thumbnail img-fluid thumbnail-img" src="${img.changeName }" alt="" >
						</a>
						<div class="portfolio-caption">
							<h4 class="thumnail-title">${img.boardTitle }</h4>
							<p class="text-muted thumnail-writer">${img.boardWriter }</p>
						</div>
					</div>
			</c:forEach>
				</div>
